...
// Load Saved Antiretroviral Medicines for Patient $_GET['code']
...
$sql = "SELECT antiretro_treatments.medicine as medicineid, medicines.Name as Medicine, StartDate, EndDate FROM antiretro_treatments, medicines WHERE antiretro_treatments.Medicine = medicines.ID AND PatientCode=".$_GET['code']." GROUP BY StartDate";
...
for ($i=0; $i<count($corrected2); $i++)
{
	$sql = "INSERT INTO `antiretro_treatments_compliance` VALUES('".$_GET['code']."', '".$corrected2[$i]['schema']."', '".$corrected2[$i]['start']."', '".$corrected2[$i]['end']."', '".$corrected2[$i]['compliance']."', '".$corrected2[$i]['reason1']."', '".$corrected2[$i]['reason2']."', '".$corrected2[$i]['notes']."')";
	echo $sql."<BR>";
	execute_query($sql);
}
?>