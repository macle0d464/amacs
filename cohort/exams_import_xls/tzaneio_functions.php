...
function get_bio_data($excel) {
	...
	for ($i=3; $i <= $excel->sheets[0]['numRows']; $i++) {
		for ($j=5; $j <= $excel->sheets[0]['numCols']; $j++) {
			$exceldata[$k] = array();
			$exceldata[$k]['exam_cell'] = clean_cell($excel->val($i, 2));
			$lower_upper_cell = clean_cell($excel->val($i, 3));
			...
			if ($exceldata[$k]['exam_cell'] == 'HDL χοληστερόλη') {
				// Dirty fix!
				$exceldata[$k]['lower'] = "";
				$exceldata[$k]['upper'] = 35; 				
			}
			$units_cell = $excel->val($i, 4);
