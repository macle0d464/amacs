...
function get_bio_data($excel) {
	...
	while ($x <= $excel->sheets[0]['numRows']) {
		$exceldata[$i] = array();
		$exceldata[$i]['index'] = $x;
		$exceldata[$i]['date_raw'] = $excel->raw($x, 4);
		$exceldata[$i]['date_cell'] = $excel->val($x, 4);
		$exceldata[$i]['mysql_date'] = xls2mysql_date($exceldata[$i]['date_raw']);
		$exceldata[$i]['exam_cell'] = $excel->val($x, 6);
		$exceldata[$i]['value_cell'] = $excel->val($x, 9);
		$lower_upper_cell = $excel->val($x, 12);
		$limits = explode(" - ", $lower_upper_cell);
		$exceldata[$i]['lower'] = $limits[0];
		$exceldata[$i]['upper'] = $limits[1]; 
		$units_cell = $excel->val($x, 10);
