...
<HTML><HEAD>
<TITLE>Καταχώρηση Λοιπών παρακλινικών Εξετάσεων Ασθενούς</TITLE>
...
{ 
    ...
    <TABLE>
    <TR>
    <TD align=left><b><u>Εξέταση</u></b><BR>&nbsp;</TD>
    <TD>&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; <b><u>Τιμή</u>&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;<u>Μονάδες</u> &nbsp; &nbsp; &nbsp; &nbsp; </b><BR>&nbsp;</TD>
...
//	$exams_sql = "SELECT laboratory_codes.Code, laboratory_codes.Description, laboratory_codes.Measurement, laboratory_codes.Upper, laboratory_codes.Lower, units.ID, units.Unit FROM laboratory_codes, units WHERE units.ID=laboratory_codes.Unit1";
    ...
    <TD><? if ($row['Code'] == 'FIBRO') {echo "Stiffness "; } ?> <INPUT size=8 name=<?echo $row['Code']?> onchange="document.all['<? echo $row['Code'];?>_Ratio'].innerHTML = this.value/document.all['<? echo $row['Code']?>_Upper'].value;">&nbsp; 
...
<?php
$sql = "SELECT exams_other.ExamDate, other_exams_list.Description, exams_other.Value, exams_other.ExtraValue, exams_other.Lower, exams_other.Upper, units.Unit, exams_other.Code FROM exams_other, other_exams_list, units WHERE other_exams_list.Code=exams_other.Code AND exams_other.Unit=units.ID AND exams_other.PatientCode=".$_GET['code'];
$result = execute_query($sql);
echo mysql_error();
    echo "<form method=post action='delete_bio_exams.php'>";
    echo "<input type=hidden name=count value=".mysql_num_rows($result).">";
    echo "<input type=hidden name=examtable value=exams_other>";
